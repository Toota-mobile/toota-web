...
function DriverSignup() {
  const dropdownRef = useRef(null); // Fixed variable name
  ...
  }, [phoneNumber, selectedCountry]); // Added dependencies
  ...
  const handleChange = (e) => {
    ...
    if (name === "password" || name === "confirmPassword") {
      checkPasswordMatch(
        ...
        name === "confirmPassword" ? value : formData.confirmPassword // Fixed variable name
      );
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.password !== formData.confirmPassword) { // Fixed typo
    ...
    try {
      const response = await fetch("https://toota-web.onrender.com/auth/signup/driver/", { // Fixed URL and content-type
        method: "POST",
        headers: {
      ...
      // If backend returned a non-2xx, extract all messages and throw
      if (!response.ok) {
        const messages = [];
        if (data.error) {
      ...
      // Redirect on verification request
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <div className="flex-grow flex flex-col justify-center items-center p-6 bg-cover bg-center" style={{ backgroundImage: "url('/Images/vcd.jpg')" }}>
        <div className="bg-white bg-opacity-80 p-8 rounded-lg shadow-lg w-full max-w-md">
          ...
          <h2 className="text-2xl font-bold text-center text-orange-500 mb-6">Driver Sign Up</h2>
